...
const _ = require('underscore');//辅助函数
...
module.exports = function proxy(app,api,config,options) {
      ...
      proxy: function(opt, config) {
        ...
        return new Promise((resolve) => {
          Promise.all(reqs).then((data) => {
            resolve(_.size(destObj)==1?destObj['body']:destObj)
          });
        })
      },
      /**
  ...
  function analyUrl(ctx, path) {
   ...
   let preUrl=api[urlReg[0]]?api[urlReg[0]]:$C.host;
    switch (urlReg.length) {
      case 1:
        ...
        break;
      case 2:
        url = fixUrl(preUrl, urlReg[1]);
        method = ctx.method;
        break;
      case 3:
        url = fixUrl(preUrl, urlReg[2]);
