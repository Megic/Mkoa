...
module.exports = function ($this, $M) {
    ...
    main['add'] = function *() {
        ...
        if (check.status) {/*通过验证*/
            ...
            if (where.id) {/*存在数据ID更新改数据*/
                res = yield $M.D('user2').update($M.POST, {where: where});
                resData = $M.POST;
            } else {/*新增*/
                res = yield $M.D('user2').build($M.POST).save();
                resData = res;
            }
    ...
    main['findOne'] = function *() {
        ...
        var res = yield $M.D('user2').sync().findOne({where: where}, {raw: true});
        $this.success(res);
    };
    ...
    main['findAll'] = function *() {
        ...
        var where = {};
        var res = yield $M.D('user2').findAndCountAll({
        where: where,
        limit: perPages,
    ...
    main['delete'] = function *() {
        ...
        if (where.id)yield $M.D('user2').destroy({where: where});
        $this.success();
    };
